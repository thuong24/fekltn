...
export default function NotificationsPage() {
  ...
  const fetchNotifications = async () => {
    try {
      ...
      // res.data.data is { notifications: [], total, unreadCount, ... }
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex justify-between items-end">
        <div>
          <h2 className="text-4xl font-black tracking-tight text-[var(--text-color)]">Thông báo</h2>
      ...
      <div className="space-y-4">
        {notifications.length === 0 ? (
          ...
          notifications.map((n) => (
            <div 
              key={n.id} 
              className={`glass-panel p-6 border-l-4 transition-all hover:scale-[1.01] cursor-pointer ${n.isRead ? 'border-transparent opacity-70' : 'border-indigo-500 shadow-xl shadow-indigo-500/5'}`}
            ...
            >
              ...
              {n.link && (
                <div className="mt-4 pt-4 border-t border-[var(--glass-border)] flex justify-end">
                   <a href={n.link} className="text-xs font-bold text-indigo-500 hover:bg-indigo-500/10 px-3 py-1.5 rounded-lg transition-colors flex items-center gap-2">
                      Xem chi tiết
                      <svg className="w-3.5 h-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M5 12h14m-7-7l7 7-7 7"/></svg>
                   </a>
                </div>
              )}
            </div>
